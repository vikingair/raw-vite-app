import React, { ChangeEvent, CSSProperties, Reducer, useCallback, useMemo, useReducer } from 'react';
...
type ArticleAction = { type: 'CLICK' } | { type: 'COLOR_CHANGE'; color: string };
...
export const Article: React.FC<ArticleProps> = ({ data }) => {
    ...
    const onChange = useCallback(
        (e: ChangeEvent<HTMLInputElement>) => dispatch({ type: 'COLOR_CHANGE', color: e.target.value }),
        []
    );

    const outerStyle = useMemo<CSSProperties | undefined>(() => (zoom ? zoomStyles : undefined), [zoom]);

    return (
        <div style={outerStyle ? { ...outerStyle, backgroundColor: color } : undefined}>
            <img
                onClick={onClick}
